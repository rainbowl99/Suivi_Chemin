...
% Variables
  xf=inp(1); yf=inp(2); thetaf=inp(3); Xf=[xf;yf;thetaf]; % etat du vehicule leader (indexé par f pour "front")
  xb=inp(4); yb=inp(5); thetab=inp(6); Xb=[xb;yb;thetab]; % etat du vehicule suiveur (indexé par b pour "back")
  
% Calcul du point Xf_proj de projection sur la courbe pour le véhicule leader - Algorithme "Brut force"  
  D=2;
  diam=5;
  opt= 1e+6;
  ray_max=5;
 ...
 % Calcul du point Xb_proj de projection sur la courbe pour le véhicule suiveur - Algorithme "Brut force"  
  D=2;
  diam=5;
  opt= 1e+6;
  ray_max=5;
